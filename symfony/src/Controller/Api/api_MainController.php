...
class api_MainController extends AbstractController
{
    ...
    #[Route('/api/home/carousel/', name: 'api_carousel', methods: ['GET'])]
    public function api_carousel(Request $request, Scene1Repository $repo, SceneD1Repository $repo2, Scene2Repository $repo3, SceneD2Repository $repo4): JsonResponse
    {
        try {
            $user = $this->getUser();
            ...
            $allScenes = []; 
            foreach ($repositories as $repository) {
                $scenes = $repository['repo']->findBy([], ['updatedAt' => 'DESC'], $limit);
                foreach ($scenes as $scene) {
                    $allScenes[] = [
                        'id' => $repository['prefix'] . $scene->getId(),
                        'updatedAt' => $scene->getUpdatedAt() ? $scene->getUpdatedAt()->format('Y-m-d H:i:s') : null,
            ...
            $lastScenes = array_slice($allScenes, 0, $limit);

            return new JsonResponse($lastScenes, Response::HTTP_OK);
        } catch (\Exception $e) {
            return new JsonResponse(['error' => 'An error occurred while fetching the carousel.'], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
}
